...
router.get("/", async (req, res) => {
  try {
    ...
    const formattedChefs = chefs.map((chef) => ({
      ...chef,
      history_worksite: chef.history_worksite
        ? JSON.parse(chef.history_worksite)
        : [],
    }));
...
router.get("/:user_id", async (req, res) => {
  try {
    ...
    chef.history_worksite = chef.history_worksite
      ? JSON.parse(chef.history_worksite)
      : [];
...
router.post("/", async (req, res) => {
  try {
    const { firstName, lastName, email, years_experience } = req.body;
    ...
    try {
      ...
      const formattedChef = {
        ...newChef,
        history_worksite: newChef.history_worksite
          ? JSON.parse(newChef.history_worksite)
          : [],
      };
  ...
  try {
    ...
    try {
      ...
      if (years_experience !== undefined) {
        // Ajouter " ans" à la valeur years_experience
        const formattedExperience = `${years_experience} ans`;
        await connection.query(
          `
                    UPDATE chef 
      ...
      const formattedChef = {
        ...updatedChef,
        history_worksite: updatedChef.history_worksite
          ? JSON.parse(updatedChef.history_worksite)
          : [],
      };
